...
# Функция генерации и сохранения РП-транскрипта переписки в файл
async def save_ticket_transcript(channel: discord.TextChannel, author_mention: str, ticket_type: str):
    ...
    try:
        with open(filename, "w", encoding="utf-8") as f:
            ...
            # Скачиваем до 500 последних сообщений из чата тикета (хватит с запасом)
            messages = []
            ...
            for msg in messages:
                ...
                # Игнорируем сообщения от самого бота с кнопками, чтобы файл не забивался мусором
...
class VerdictModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed.add_field(name="专 Подпись должностного лица:", value=f"`{signature}`", inline=False)
        ...
        # АВТОМАТИЧЕСКАЯ ВЫГРУЗКА ТРАНСКРИПТА ПРИ ОДОБРЕНИИ И ЗАКРЫТИИ ТИКЕТА
