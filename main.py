...
def build_doc_embed(target: discord.Member, u_data: dict, doc_type: str):
    if doc_type == "паспорт" and u_data.get("passport_data"):
    ...
    elif doc_type == "лицензии" and u_data.get("license_data"):
        d = u_data["license_data"]
        embed = discord.Embed(title="🚗 ВОДИТЕЛЬСКОЕ УДОСТОВЕРЕНИЕ", color=discord.Color.blue())
    ...
    elif doc_type == "медкарта" and u_data.get("med_exam"):
        d = u_data["med_exam"]
...
async def profile_slash(interaction: discord.Interaction, пользователь: discord.Member = None):
    ...
    m_txt = u_data.get("married_data") if u_data.get("married_data") else "Нет"
...
async def show_doc_slash(interaction: discord.Interaction, документ: str, кому: discord.Member):
    ...
    if embed is None:
        await interaction.response.send_message(f"❌ У вас ещё не оформлен документ (`{dtype}`). Зайдите в Центр Услуг.", ephemeral=True)
        return
        
    await interaction.response.send_message(f"✅ Вы показали документ игроку {comm.mention}", ephemeral=True)
...
# --- КОМАНДА МЕДИКОВ ---
ROLE_MEDIC_ID = 1523265245030649866  # ЗАМЕНИТЕ НА ID ВАШЕЙ РОЛИ МЕДИКОВ
...
@bot.event
async def on_ready(): 
    print(f"🤖 Бот {bot.user.name} успешно запущен. Все текстовые команды (!меню, !баланс) активны!")
